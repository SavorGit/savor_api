...
class GoodsModel extends BaseModel{
	...
	public function getAllShopGoods($keywords='',$start,$size){
        ...
        $all_sql = "(select id,name,price,'0.00' as line_price,'0' as amount,media_id,cover_imgmedia_ids as cover_imgs,type,add_time from savor_smallapp_goods 
        where type=10 and status=2 and show_status= 1 and start_time<= '{$nowtime}' and end_time>= '{$nowtime}' order by id desc) union 
        (select id,name,price,line_price,amount,'0' as media_id,cover_imgs,type,add_time from savor_smallapp_dishgoods where type=22 and status=1 order by id desc)";
        $where = '';
