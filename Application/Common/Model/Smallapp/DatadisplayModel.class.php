...
class DatadisplayModel extends BaseModel{
	...
	public function recordDisplaynum($datalist,$area_id,$type=0){
            ...
            if($v['type']==1){
                ...
                if(!empty($res_record)){
                    ...
                    $data = array('forscreen_id'=>$forscreen_id,'resource_id'=>$res_info['resource_id'],'area_id'=>$area_id,
                        'oss_addr'=>$oss_addr,'display_num'=>1,'type'=>$type,'add_date'=>$add_date);
                    $this->add($data);
                }
            }else{
                if($type==0)    $type=1;
                $ads_id = $v['ads_id'];
                $res_record = $this->getInfo(array('ads_id'=>$ads_id,'type'=>$type,'area_id'=>$area_id,'add_date'=>$add_date));
                if(!empty($res_record)){
                    $this->where(array('id'=>$res_record['id']))->setInc('display_num',1);
                }else{
                    ...
                    $data = array('ads_id'=>$ads_id,'media_id'=>$res_adsinfo[0]['media_id'],'resource_name'=>$res_adsinfo[0]['name'],'oss_addr'=>$res_adsinfo[0]['oss_addr'],
                        'area_id'=>$area_id,'display_num'=>1,'type'=>$type,'add_date'=>$add_date);
                    $this->add($data);
                }
            }
        }
