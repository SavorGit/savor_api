...
//            $res_data = $m_hotelgoods->getGoodsList($fields,$where,'g.id desc',"$start,$pagesize");
    ...
    public function distributionGoodsList(){
        ...
        $start = ($page-1)*$pagesize;
        $m_goods = new \Common\Model\Smallapp\DishgoodsModel();
        $res_goods = $m_goods->getDataList('id,name,price,line_price,cover_imgs,desc,desc2,distribution_config',array('type'=>45,'status'=>1),'sort desc,id desc',$start,$pagesize);
        $oss_host = get_oss_host();
        if($res_goods['list']){
            foreach ($res_goods['list'] as $v){
                $img_url = '';
